...
import { PluggyAccount, PluggyItem, PluggyTransaction } from '../pluggy/PluggyClient.js';
...
export function resolveDocumentParameterName(credentials: Array<{ name: string; label?: string; validation?: string }> = []) {
  ...
  return params.transactions.map((transaction) => {
    ...
    return {
      ...
      kipo: {
        ...
        providerTransactionId,
        institutionName: params.institutionName,
        modalidade: mapOperationType(transaction.operationType, transaction.paymentData?.paymentMethod),
        status: transaction.status ?? 'POSTED',
      },
    };
  });
